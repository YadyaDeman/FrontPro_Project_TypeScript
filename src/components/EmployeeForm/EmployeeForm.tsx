...
import { CardFormComponent, Checkbox, CheckboxContainer, CheckboxLabel, EmployeeFormComponent, ErrorCheckboxMessage, FormWrapper } from "./styles";
...
function EmployeeForm() {
  ...
  const validationSchema = Yup.object().shape({
    ...
    [EMPLOYEE_FIELD_NAMES.AGE]: Yup.number()
      ...
      .required('Required field')
       .max(110, 'Max 3 symbols'),
      //.test('check length', 'Max 3 symbols', ageValue => String(ageValue).length <= 3),
    ...
    <FormWrapper>
      <EmployeeFormComponent onSubmit={formik.handleSubmit}>
        ...
        <CheckboxContainer>
          ...
          />
          <CheckboxLabel htmlFor='agreement-id'>Say Agree</CheckboxLabel>
        </CheckboxContainer>

        <ErrorCheckboxMessage>{formik.errors[EMPLOYEE_FIELD_NAMES.AGREEMENT]}</ErrorCheckboxMessage>
